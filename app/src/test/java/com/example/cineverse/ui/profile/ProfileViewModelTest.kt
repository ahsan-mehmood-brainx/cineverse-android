...
class ProfileViewModelTest {
    ...
    fun validate_blankName_returnsBlankNameError() {
        assertThat(viewModel.validate("   ", "bio")).isEqualTo(ProfileFormError.BlankName)
    }

    @Test
    fun validate_nameOverLimit_returnsNameTooLongError() {
        val longName = "a".repeat(ProfileViewModel.MAX_NAME_LENGTH + 1)
        assertThat(viewModel.validate(longName, "")).isEqualTo(ProfileFormError.NameTooLong)
    }

    @Test
    fun validate_bioOverLimit_returnsBioTooLongError() {
        val longBio = "a".repeat(ProfileViewModel.MAX_BIO_LENGTH + 1)
        assertThat(viewModel.validate("Alex", longBio)).isEqualTo(ProfileFormError.BioTooLong)
    }

    @Test
    fun validate_validInput_returnsNull() {
        assertThat(viewModel.validate("Alex", "Loves sci-fi")).isNull()
    }

    @Test
    fun save_validInput_trimsAndPersists() = runTest(testDispatcher) {
        val saved = viewModel.save("  Alex  ", "  Loves sci-fi  ")
        advanceUntilIdle()

        assertThat(saved).isTrue()
        assertThat(repository.saved).isEqualTo(Profile(displayName = "Alex", bio = "Loves sci-fi"))
    }

    @Test
    fun save_invalidInput_doesNotPersist() = runTest(testDispatcher) {
        val saved = viewModel.save("   ", "bio")
        advanceUntilIdle()
