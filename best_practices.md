...
All Android pull requests must follow these standards to ensure modern, production-safe, crash-free, maintainable, scalable, and reusable Kotlin code (Jetpack Compose and/or XML Views) — regardless of whether the code was authored by a human, an AI assistant, or a combination.

Compliance reviews use three checklists as the source of truth:

- `pr_compliance_checklist.yaml` — main code-quality sections (the structure of this document).
- `testing_compliance_checklist.yaml` — dedicated, detailed testing requirements (evaluated as a separate mandatory phase).
- `prd_compliance_checklist.yaml` — PRD requirements and PR-vs-PRD alignment (evaluated as a separate mandatory phase).

Each applicable section is evaluated in **strict compliance mode**: every deviation from these standards must be reported as a violation with actionable fixes.
...
## 33. AI-Assisted Code Quality and Consistency

AI assistants (Copilot, Cursor, Claude, ChatGPT, Gemini, Android Studio's built-in AI, and others) are useful accelerators, but AI-authored code must meet the same quality bar as human-authored code and must remain coherent with the surrounding project.

### Requirements

- Code style, formatting, and structural choices must be internally consistent within a PR. Do not commit a mix of patterns copy-pasted from multiple AI sessions.
- No hallucinated APIs, package names, method signatures, class names, constants, or imports that do not exist.
- All imports must resolve to real dependencies declared in `build.gradle`/`build.gradle.kts`/version catalogs. Remove phantom imports and unused imports.
- No leftover AI narration or boilerplate comments (`// Here is the code`, `// This function does...`, `// Sure! Here's...`, `// Explanation:`).
- No placeholder identifiers or text (`foo`, `bar`, `example1`, `dummyValue`, lorem ipsum, `TODO implement`).
- No stray try/catch, `println`, exploratory scaffolding, or dead branches left by AI.
- Naming, package placement, and architectural choices must match the surrounding codebase — not generic AI defaults.
- Reuse existing project helpers, managers, extensions, and reusable Composables/Views instead of AI reintroducing duplicates.
- Business logic must reflect the actual product requirements and PRD, not a generic textbook implementation.
- Regex, JSON parsing, date formatting, currency handling, and validators generated by AI must be verified against real inputs.
- AI-generated tests must assert meaningful behavior — no empty bodies, no `assertTrue(true)`.
- Trim overly verbose or over-defensive AI code to match project pragmatism.
- Dependencies added by AI must be justified, used, and non-duplicate.
- Deprecated APIs suggested by AI (`AsyncTask`, `onActivityResult`, `findViewById` where ViewBinding is expected, `startActivityForResult`) must be replaced with current equivalents.
- Code must show evidence of human review and adaptation.

---

## PRD Compliance (Separate Checklist)

Every new feature, bug fix, enhancement, refactor, or update must be traceable to a clear, professional PRD (or equivalent ticket/root-cause note for small fixes). The detailed requirements live in **`prd_compliance_checklist.yaml`** and are evaluated as a separate, important review phase. A professional PRD must include, at minimum: problem statement, goals/non-goals, target users, user stories, explicit and testable acceptance criteria, functional and non-functional requirements, primary/alternate/error flows, UI/UX references with all screen states, data/API contract, edge cases, analytics and success metrics, rollout/migration plan, dependencies and risks, and ownership/version. The PR implementation must align with the PRD; any deviation must be documented and justified.

---

## Testing Compliance (Separate Checklist)

Every PR that changes behavior must include the applicable tests **in the same PR** as the implementation. The detailed requirements live in **`testing_compliance_checklist.yaml`** and are evaluated as a separate, mandatory review phase covering: TDD discipline, unit tests, model/serialization tests, Compose/View UI tests, screenshot/visual tests, integration and functional tests, API/network tests, edge cases and error paths, and coverage/CI. Every minor and major feature, change, edge case, and UI update must be tested and verified.
...
## F. Accessibility, Localization, and Observability

- Provide `contentDescription` for icon-only/gesture-only interactive elements; mark purely decorative images/icons as `null`/`importantForAccessibility="no"` so TalkBack skips them.
- Support font-scale changes (`sp` units, not `dp`, for text) — avoid fixed text sizes that break under large accessibility font settings.
- Ensure sufficient color contrast for text/icons, including in dark theme.
- Localize all user-facing strings via `strings.xml`; do not hardcode strings in Composables/layouts/Kotlin code.
- Format dates, numbers, and currencies using locale-aware helpers (`DateTimeFormatter`, `NumberFormat`) instead of manual string building.
- Support RTL layouts (`start`/`end` instead of `left`/`right` in both XML and Compose `Modifier`s) where the project targets RTL locales.
- Instrument new features with the project's analytics, logging, and crash-reporting pipelines (Firebase Analytics/Crashlytics or the project's equivalent).

## G. Additional Observations (Reviewer Autonomy)

The reviewer must go beyond the checklists and this document. Any meaningful improvement, risk, or concern in the PR must be reported under `additional_observations`/`others` using senior Android engineering judgment — security, performance, accessibility, localization, analytics, dependency risk, API contract, data safety, navigation correctness, Gradle/build-config concerns, and long-term maintainability — even when no specific rule names the concern. Every such observation must include a concrete Suggested Fix.
- Keep imports clean and organized.