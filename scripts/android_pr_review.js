...
const STATIC_RULES = [
  ...
  {
    id: 'ai_generated_code_quality',
    pattern: /\/\/\s*(Here'?s|Here is|This function|This code|Sure!|Explanation:|As an AI)/i,
    issue: 'Leftover AI narration/boilerplate comment left in the diff.',
    standard: 'AI-Assisted Code Quality and Consistency',
  },
  {
    id: 'ai_generated_code_quality',
    pattern: /\bassertTrue\s*\(\s*true\s*\)/,
    issue: 'Trivially-true assertion (`assertTrue(true)`) — the test does not verify real behavior.',
    standard: 'AI-Assisted Code Quality and Consistency',
  },
];
...
function normalizeSections(modelSections, expectedSections, changedFiles, staticFindings, group) {
  ...
  return expectedSections.map(section => {
    ...
    const inferred = inferApplicability(section.id, changedFiles, DEFAULT_APPLICABLE_GROUPS.has(group));
...
const DEFAULT_APPLICABLE_GROUPS = new Set(['checklist', 'testing', 'prd']);
...
async function callOpenAI({ phase, sections, sourceDoc, diff, files, fileSnippets }) {
  ...
  const PHASE_INSTRUCTIONS = {
    checklist: `PHASE 1 — PR COMPLIANCE CHECKLIST (HIGHEST PRIORITY)
...
${sectionList}`,
    best_practices: `PHASE 2 — ADDITIONAL BEST PRACTICES (SECONDARY, AFTER CHECKLIST)
...
Sections to evaluate:
${sectionList}`,
    testing: `PHASE 3 — TESTING COMPLIANCE (SEPARATE, MANDATORY PHASE)

Evaluate the PR against testing_compliance_checklist.yaml. This is independent of Phase 1/2.
For EACH of the ${sections.length} testing sections below:
- Determine which test types the changed behavior requires (unit / model-serialization / Compose-or-View UI / screenshot / integration / API-network / edge-case) and check whether the PR diff actually adds or updates them.
- If the PR changes any logic, model, screen, or flow and ships with no corresponding test, that is a violation — do not assume tests exist elsewhere unless visible in the diff or changed-files list.
- Flag empty/trivial test bodies, tests with no meaningful assertion, or skipped (\`@Ignore\`) tests without justification.
- Only mark applicable=false for a section the PR genuinely does not touch (e.g. no UI change -> ui_change_and_visual_tests may be N/A).
- For each gap, the fix must name the exact test to add and what it should assert.

Sections to evaluate:
${sectionList}`,
    prd: `PHASE 4 — PRD COMPLIANCE (SEPARATE, IMPORTANT PHASE)

Evaluate the PR against prd_compliance_checklist.yaml. This is independent of Phase 1/2/3.
For EACH of the ${sections.length} PRD sections below:
- Check whether the PR description contains a reachable, non-placeholder PRD/ticket/incident reference.
- For feature PRs, check whether the referenced PRD (if attached/quoted) is professionally structured (problem, goals, acceptance criteria, flows, UI states, data contract, edge cases, analytics, rollout, risks, ownership).
- Check whether the implementation aligns with the PRD's acceptance criteria and required screen states, and flag scope creep or scope gaps.
- Check whether any deviation from the PRD is explicitly documented and justified in the PR description.
- Trivial chores (formatting, dependency bump, doc-only changes) may be marked applicable=false with a one-line reason.

Sections to evaluate:
${sectionList}`,
  };

  const phaseInstructions = PHASE_INSTRUCTIONS[phase] || PHASE_INSTRUCTIONS.best_practices;
...
function formatComment(checklistSections, bestPracticeSections, testingSections, prdSections) {
  const countOf = sections => sections.filter(s => s.applicable).reduce((n, s) => n + s.violation_count, 0);
  const checklistViolations = countOf(checklistSections);
  const bpViolations = countOf(bestPracticeSections);
  const testingViolations = countOf(testingSections);
  const prdViolations = countOf(prdSections);
  const totalViolations = checklistViolations + bpViolations + testingViolations + prdViolations;

  let md = `${MARKER}\n`;
  md += `# Android PR Compliance Review\n\n`;
  md += `**Review order:** 1) \`pr_compliance_checklist.yaml\` → 2) \`best_practices.md\` (Sections A–E) → 3) \`testing_compliance_checklist.yaml\` → 4) \`prd_compliance_checklist.yaml\`\n\n`;
  md += `**Total violations:** ${totalViolations} (Checklist: ${checklistViolations}, Best practices: ${bpViolations}, Testing: ${testingViolations}, PRD: ${prdViolations})\n\n`;

  let sr = 1;
  md += `## Evaluation Summary\n\n`;
  md += `### PR Compliance Checklist (\`pr_compliance_checklist.yaml\`)\n\n`;
  md += formatSummaryTable(checklistSections, sr);
  sr += checklistSections.length;

  md += `\n### Additional Best Practices (\`best_practices.md\`)\n\n`;
  md += formatSummaryTable(bestPracticeSections, sr);
  sr += bestPracticeSections.length;

  md += `\n### Testing Compliance (\`testing_compliance_checklist.yaml\`)\n\n`;
  md += formatSummaryTable(testingSections, sr);
  sr += testingSections.length;

  md += `\n### PRD Compliance (\`prd_compliance_checklist.yaml\`)\n\n`;
  md += formatSummaryTable(prdSections, sr);
  ...
  md += `## Phase 3 — Testing Compliance Details\n\n`;
  for (const s of testingSections) md += formatSectionDetail(s);

  md += `## Phase 4 — PRD Compliance Details\n\n`;
  for (const s of prdSections) md += formatSectionDetail(s);
...
async function main() {
  ...
  const testingChecklist = readFile('testing_compliance_checklist.yaml');
  const prdChecklist = readFile('prd_compliance_checklist.yaml');
  if (!checklist || !bestPractices || !testingChecklist || !prdChecklist) {
    throw new Error('best_practices.md, pr_compliance_checklist.yaml, testing_compliance_checklist.yaml, and prd_compliance_checklist.yaml must all exist in repo root');
  }
  ...
  const testingSectionDefs = parseChecklistSections(testingChecklist);
  const prdSectionDefs = parseChecklistSections(prdChecklist);
  ...
  console.log('Phase 3: Reviewing testing_compliance_checklist.yaml...');
  const testingReview = await callOpenAI({
    phase: 'testing',
    sections: testingSectionDefs,
    sourceDoc: { label: 'testing_compliance_checklist.yaml', content: testingChecklist },
    diff,
    files: filesOutput,
    fileSnippets,
  });

  console.log('Phase 4: Reviewing prd_compliance_checklist.yaml...');
  const prdReview = await callOpenAI({
    phase: 'prd',
    sections: prdSectionDefs,
    sourceDoc: { label: 'prd_compliance_checklist.yaml', content: prdChecklist },
    diff,
    files: filesOutput,
    fileSnippets: `PR title: ${PR_TITLE}\nPR description:\n${PR_BODY || '(empty)'}`,
  });
  ...
  const testingSections = normalizeSections(
    testingReview.sections,
    testingSectionDefs,
    changedFiles,
    [],
    'testing',
  ...
  const prdSections = normalizeSections(
    prdReview.sections,
    prdSectionDefs,
    changedFiles,
    [],
  ...
  const comment = formatComment(checklistSections, bestPracticeSections, testingSections, prdSections);
  await upsertComment(comment);

  const total = [...checklistSections, ...bestPracticeSections, ...testingSections, ...prdSections]
